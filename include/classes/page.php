...
class Page {
	...
	function unregister($varname) {
		...
		if(array_key_exists($varname, $_REQUEST)){unset($_REQUEST[$varname]);}
	}

	function unregisterAll(){
		foreach($this->vars as $v){
			$this->unregister($v[varname]);
		}
	}
	...
	function printTextbox($v, $attr, $disp_type = "form") {
       ...
       function printRadio($v, $attr, $disp_type = "form") {
		...
		if($disp_type == "form"){
                        foreach($choices as $c) {?>
                                <input type="radio" name="<?=$v?>" value="<?=$c[value]?>" <?if($_REQUEST[$v] == $c[value]) echo "CHECKED";?>
                                ><?=$c[text]?><br/><?;
                        }
                } else {
                        foreach($choices as $c) {
